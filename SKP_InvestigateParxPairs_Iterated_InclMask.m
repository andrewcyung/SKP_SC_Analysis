...
dest_path = [rootpath 'Group Results\06-Correlations\02-InclMaskHisto\'];

mod_sf = cell(1,n_pair);

% re-mask the parameter maps with the inclusion mask from histology images
% (only the pixels that are entirely out of the section .tif inclusion masks are
% included)
mask_path_prefix = '\06-Transformation\01-HistologyParMaps\02-Results\HistoParMap_';
mask_path_suffix = '_MBP_InclMask.tif';
mod_sf{1}=MaskStorageFrame(sf,rootpath,mask_path_prefix,mask_path_suffix,{'MWF','Dtrans','MBP_IntegODThresh','MBP_AreaFraction'});
mod_sf{2}=mod_sf{1};
mod_sf{7}=mod_sf{1};
mod_sf{8}=mod_sf{1};

mask_path_prefix = '\06-Transformation\01-HistologyParMaps\02-Results\HistoParMap_';
mask_path_suffix = '_EC_InclMask.tif';
mod_sf{3}=MaskStorageFrame(sf,rootpath,mask_path_prefix,mask_path_suffix,{'MWF','Dtrans','EC_IntegODThresh','EC_AreaFraction'});
mod_sf{4}=mod_sf{3};
mod_sf{9}=mod_sf{3};
mod_sf{10}=mod_sf{3};

mask_path_prefix = '\06-Transformation\01-HistologyParMaps\02-Results\HistoParMap_';
mask_path_suffix = '_Axon_InclMask.tif';
mod_sf{5}=MaskStorageFrame(sf,rootpath,mask_path_prefix,mask_path_suffix,{'Dlong','Axon_AreaFraction','Axon_IntegODThresh'});
mod_sf{6}=mod_sf{5};
mod_sf{9}=mod_sf{3};
mod_sf{10}=mod_sf{3};
...
%             status{i_subject}.isReg.AxonSum2MRI
            ...
            StartPointViewCoordinator(mod_sf{i_pair},storage_layout,view_categories,requested_members,disp_attributes,h_scatter_list,h_scatterlegend_list,scatter_parxname_list,h_parMapView_list,parMapView_name_list,h_upstreamView_list,rootpath)

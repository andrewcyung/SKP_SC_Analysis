...
for j=8:8
    ...
    HistoROI_basepath = [rootpath id '\' '03-Segmentation\02_Histology\03_Set 3 - Eriochrome_10x\08-Ventral damaged vs intact\'];
    ...
    for i=1:1
        ...
        % load histology ROI (one from each section corresponding to slice
        i_foundsection = 1;
        for r=1:n_sections
            % read histology image data
            section_basename = num2str(startindex-1+r);
            filename = [HistoROI_basepath section_basename];
            if exist([filename '.tif'], 'file') == 2
                section_im{i_foundsection} = imread(filename);
                if exist([filename '.ov1'], 'file') == 2
                    section_ov1{i_foundsection} = imread(filename);
                end
                if exist([filename '.ov2'], 'file') == 2
                    section_ov2{i_foundsection} = imread(filename);
                end
                if exist([filename '.ov3'], 'file') == 2
                    section_ov3{i_foundsection} = imread(filename);
                end

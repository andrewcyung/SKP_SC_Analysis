...
%   .coord          the position or ordinate of the corresponding members of .data
...
%           (fieldnames include .id, .data, .srcimpath, .tfmparx2srcim).

            accum_parvec = [];
            if ~strcmp(groupid,'collapsed')
                found_flag = cellfun(@(element)strcmp(element.id,groupid),storageframe);
                group_iterator = find(found_flag);
            else
                group_iterator = 1:n_group;
            end
